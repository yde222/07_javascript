...
// encodeURI : URI(인터넷에 있는 자원을 나타내는 유일한 주소)를 문자열로 전달받아 이스케이프 처리를 위해 인코딩한다.
...
// decodeURI : 인코딩 된 URI를 인수로 전달 받아 이스케이프 처리 이전으로 디코딩한다.
const dec = decodeURI(enc);
console.log(dec); // http://greedy.com?name=홍길동&job=student

// ncodeURIComponent : URI 구성 요소를 인수로 전달 받아 알파벳, 0~9의 숫자, - _ . ! ~ * ’ ( ) 문자는 제외하고 인코딩한다.
...
// decodeURIComponent : 인코딩 된 URI를 인수로 전달 받아 이스케이프 처리 이전으로 디코딩한다.
