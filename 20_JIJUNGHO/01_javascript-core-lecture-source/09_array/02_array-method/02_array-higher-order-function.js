...
console.log(`기본값(문자) 정렬 전 numbers : ${numbers}`);
numbers.sort(); // 정렬 (오름차순)
console.log(`기본값(문자) 정렬 후 numbers : ${numbers}`);

// sort의 기본 정렬은 문자 정렬이므로 필요하다면 정렬기준을 콜백 함수로 전달해야한다.
...
// Array.prototype.map : 배열 요소 전체를 대상으로 콜백 함수 호출 후 반환 값들로 구성 된 새로운 배열 반환한다.
...
const odds = numbers.filter((el) => el % 2); // 나머지가 1 >>>>> true
...
// Array.prototype.some : 배열 내 일부 요소가 콜백함수와 테스트를 통과하는지 확인
//                        그 결과를 boolean으로 반환
// 배열 내 10보다 큰값이 1개 이상 존재하는 지 확인
...
// Array.prototype.every : 배열 내 모든 요소가 콜백함수의 테스트를 통과하는지 확인
//                         그 결과를 boolean으로 반환
...
// Array.prototype.find : 배열을 순회하며 각 요소에 대하여 인자로 주어진 콜백함수를 실행하여 그 결과가 참인 첫번째
//                        요소를 반환, 참인 요소가 존재하지 않는다면 undefined를 반환
// Array.prototype.findIndex : 배열을 순회하며 각 요소에 대하여 인자로 주어진 콜백함수를 실행하여 그 결과가 참인 첫번째
//                             요소의 인덱스를 반환, 참인 요소가 존재하지 않는다면 -1을 반환
const student = [
  { name: "유관순", score: 90 },
  { name: "홍길동", score: 80 },
  { name: "장보고", score: 70 },
...
console.log("result :", result); // { name: '홍길동', score: 80 }
result = student.findIndex((item) => item.name === "홍길동");
console.log("result :", result); // 1

result = student.filter((item) => item.score >= 80);
console.log("result :", result); // [ { name: '유관순', score: 90 }, { name: '홍길동', score: 80 } ]
